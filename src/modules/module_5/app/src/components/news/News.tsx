...
const newsTestState = [{
  ...
  desciption: 'Ethereum is moving closer to adopting a proof-of-stake model for its network, which is less energy i',
  url: 'https://www.youtube.com/watch?v=dQw4w9WgXcQ',
}];

export const News: React.FC = () => {
  const [scroll, setScroll] = useState(0);
  const wrapperElement = document.querySelector('#news__desktop-container');
  const [maxWidth, setMaxWidth] = useState(0);
  const [clientWidth, setClientWidth] = useState(0);
  const [isStart, setIsStart] = useState(true);
  const [isEnd, setIsEnd] = useState(false);
  const [newsArray, setNewsArray] = React.useState<INews[]>(newsTestState);
  ...
  useEffect(() => {
    async function getNewsFromApi() {
      ...
      result.articles.forEach((element: INews) => {
        ...
        const { urlToImage } = element;
        const { url } = element;
        output.push({ title, description, urlToImage, url });
      });
      setNewsArray(output);
      setIsLoaded(true);
    }
